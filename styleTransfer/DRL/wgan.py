...
loader = transforms.Compose([
                             ...
                             # if we do this, we'll need to normalize it back otherwise the colours will be irregular.
])

style_img = load_image("fire.jpg")
style_features = model(style_img)

# generated_img = torch.randn(original_img.shape, device = device, requires_grad=True)
# doing copy of the original image gives better result
...
def update(fake_data, real_data):
    ...
    D_cost = 0.9*(D_fake.mean() - D_real.mean() + gradient_penalty) + 0.1*(styleLoss(fake_data,style_features)) #add style loss
    D_cost.backward()
    optimizerD.step()
